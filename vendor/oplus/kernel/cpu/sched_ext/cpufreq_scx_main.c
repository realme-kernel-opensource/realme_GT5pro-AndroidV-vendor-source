							...
							u64 prev_runnable_sum)
{
	...
	gov_trace_printk("cluster[%d] max_freq[%d] win_tl[%d]"
			"cpu_cap[%lu] divisor[%llu] next_f[%d]\n",
			cpu, freq, window_size_tl,
...
static void scx_irq_work(struct irq_work *irq_work)
{
	...
	for_each_sched_cluster(cluster) {
		...
		for_each_cpu(cpu, &cluster_online_cpus) {
			...
			prev_runnable_sum = max(prev_runnable_sum,
						srq->prev_runnable_sum);
		}
...
static void scxgov_update_freq(struct waltgov_callback *cb, u64 time, unsigned int flags)
{
	...
	for_each_cpu(cpu, policy->cpus) {
		...
		sum = wrq->prev_runnable_sum + wrq->grp_time.prev_runnable_sum;
