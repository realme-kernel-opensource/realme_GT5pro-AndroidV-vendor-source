...
/*
static void android_vh_account_for_runnable_bypass_handler(void *unused,
			struct rq *rq, struct task_struct *p, int event, int *skip)
{
	if (scx_enabled() && (event == PICK_NEXT_TASK || event == TASK_MIGRATE))
		*skip = true;
}
*/
...
/*	REGISTER_TRACE_VH(android_vh_account_for_runnable_bypass,
				android_vh_account_for_runnable_bypass_handler);
*/
	register_helper_ops();
}
